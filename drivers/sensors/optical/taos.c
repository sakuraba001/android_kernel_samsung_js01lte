...
static void sensor_power_on_vdd(struct taos_data *info, int onoff)
{
	...
	if (info->vreg_2p8 == NULL) {
		info->vreg_2p8 =regulator_get(&info->i2c_client->dev, "8226_l15");
	...
	struct device_attribute *attr, char *buf)
{
    ...
    pr_err("%s: initial offset =%d and opt_i2c_read =%d",__func__, p_offset,ret);
			...
			struct  taos_platform_data *pdata)
{
	...
	// vdd_en= of_get_named_gpio(np, "taos,vdd_en-gpio", 0);
	// ret = gpio_request(vdd_en, "vdd_en");
	// if(ret)
		// pr_err("[TAOS]%s: gpio request fail\n",__func__);
	// gpio_tlmm_config(GPIO_CFG(vdd_en, 0,
			// GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 1);
	// ret = gpio_direction_output(vdd_en, 1);
	//if (ret)
		//pr_err("[taos]%s: unable to set_direction for vdd_en [%d]\n",__func__, vdd_en);
			  ...
			  const struct i2c_device_id *id)
{
	...
	printk("AMIT taos_i2c_probe Start\n");
	...
	if (chipid != CHIP_ID || chipid != CHIP_ID1) {
		pr_err("%s: i2c read error [%X] CHIP_ID\n", __func__, chipid);
...
/* set sysfs for proximity sensor and light sensor */
	...
	err =  sensors_initialize_symlink(taos->proximity_input_dev);
	if (err) {
...
#endif
	printk("AMIT taos_i2c_probe Succres\n");
	goto done;
...
#ifdef CONFIG_OF
static struct of_device_id tm2672_match_table[] = {
...
static struct i2c_driver taos_i2c_driver = {
	.driver = {
		...
		.owner = THIS_MODULE,
		.pm = &taos_pm_ops,
		.of_match_table = tm2672_match_table,
	},
