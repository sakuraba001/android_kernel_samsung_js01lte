		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (ret != 0)
		pr_debug("[CMC624] ERROR : set main tune value faild\n");
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	ret = apply_sub_tune_value(cmc624_state.temperature, value,
		cmc624_state.cabc_mode, 0);
	if (ret != 0)
		pr_debug("[CMC624:ERROR] : set sub tune value faild\n");
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (ret != 0)
		pr_debug("[CMC624:ERROR] : set ebook value faild\n");
			...
			const char *buf, size_t size)
{
	...
	if (ret < 0)
		...
		if (value == COLOR_BLIND) {
			...
			if (ret != 0) {
				pr_debug("[CMC624:ERROR] : set blind value failed.\n");
				return size;
			}
			...
			if (ret != 0) {
				pr_debug("[CMC624:ERROR] : set negative value failed.\n");
				return size;
			}
